...
def generate_commentary_ollama(
    ...
    if is_spatial_snapshot:
        user_content = (
            ...
            "Metric Context: Stats are not available for this action."
        )
    else:
        user_content = (
            f"Level: {level}\n\n"
...
def process_event(
    ...
    # ── Determine if this is a goal / foul event ──
    goal = is_goal_event(event)
    foul = is_foul_event(event)

    # ── Build the event sequence (for goal / foul, include related events) ──
    if goal or foul:
        ...
        # Regular event — single event in the sequence
    ...
    # ── Get 360 data — ONLY from threesixty.json ──
