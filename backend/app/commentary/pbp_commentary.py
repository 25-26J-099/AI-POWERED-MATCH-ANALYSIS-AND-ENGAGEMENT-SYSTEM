...
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
# LOCATION HELPERS (StatsBomb 120├ù80 pitch)
# ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    for ollama_bin in ollama_paths:
        try:
            ...
            if result.returncode == 0:
                print(f"Γ£à Model '{model_name}' created successfully via CLI.")
                return True
        ...
        for line in r.iter_lines():
            if line:
                ...
                if "error" in data:
                    ...
                    return False
        print(f"Γ£à Model '{model_name}' created successfully via API.")
        return True
    except Exception as e:
        print(f"Γ¥î Failed to create model: {e}")
        return False
...
def main():
    print("=" * 60)
    print("ΓÜ╜ Play-by-Play Commentary Generation Pipeline")
    print("=" * 60)
    ...
    # 1) Load data
    print("\n≡ƒôª Loading event data...")
    with open(event_path, "r", encoding="utf-8") as f:
        events = json.load(f)
    print(f"   Loaded {len(events)} events.")
    
    print("≡ƒôª Loading 360 data...")
    ...
    for a in anchors:
        ...
        print(f"      [{minute}:{second:02d}] {etype} by {player} ΓÇö {a['reason']}")
    
    # 3) Build flattened streams and classify intensity
    print("\n≡ƒô¥ Building flattened event streams...")
    prompts_data = []
    ...
    if needs_llm:
        ...
        if not check_ollama_running():
            print("ΓÜá∩╕Å  Ollama is not running. Falling back to template commentary.")
        else:
            if not check_model_exists(MODEL_NAME):
                ...
                if create_model(MODEL_NAME, modelfile_path):
                    ...
                    print("ΓÜá∩╕Å  Failed to create model. Falling back to template commentary.")
            else:
                print(f"   Γ£à Model '{MODEL_NAME}' is ready.")
                llm_available = True
    else:
        print("\n≡ƒñû Skipping Ollama checks (selected anchors are template-driven).")
    
    # 5) Generate commentary
    print("\n≡ƒÄÖ∩╕Å  Generating commentary...\n")
    print("-" * 60)
    ...
    for i, pd_entry in enumerate(prompts_data):
        ...
        print(f"\n[{i+1}/{len(prompts_data)}] ΓÅ▒ {minute}:{second:02d} | "
    ...
    print(f"\nΓ£à Saved commentary to: {output_path}")
    ...
    print(f"Γ£à Saved prompts reference to: {prompts_path}")
    
    print("\n≡ƒÅü Pipeline complete!")
